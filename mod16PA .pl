...
% Anyone who is not a male is a female ( makes unknown people female)
...
step_father(X,Y) :- step_parent(X,Y), male(Y).
...
step_parent(X,Y) :- parent(X,Z), spouse(Y,Z), \+parent(X,Y).
...
aunt(X,Y) :- parent(X,Z), sibling(Z,W), spouse(W,Y), female(Y).
